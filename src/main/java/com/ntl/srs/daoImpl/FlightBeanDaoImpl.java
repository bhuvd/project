...
import com.ntl.srs.dao.FlightBeanDao;
...
public class FlightBeanDaoImpl implements FlightBeanDao{
	...
	public String createFlightBean(FlightBean flightBean) throws SQLException {
		...
		ps=con.prepareStatement("insert into SRS_TBL_flight values(?,?,?,?)");
	...
	public boolean updateFlightBean(FlightBean flightBean) throws SQLException   {
		// TODO Auto-generated method stub
			...
			ps=con.prepareStatement("update SRS_TBL_Flight set FlightName='"+flightBean.getFlightName()+"', SeatingCapacity='"+flightBean.getSeatingCapacity()+"',ReservationCapacity='"+flightBean.getReservationCapacity()+"' where flightId='"+flightBean.getFlightID()+"'");
	...
	public FlightBean findByID(String id) throws SQLException {
		...
		ps=con.prepareStatement("select * from srs_tbl_flight where flightId='"+id+"'");
		rs=ps.executeQuery();
		while(rs.next())
		{
	...
	public ArrayList<FlightBean> findAll() throws SQLException {
		...
		ps=con.prepareStatement("select * from srs_tbl_flight ");
		rs=ps.executeQuery();
		while(rs.next())
		{
